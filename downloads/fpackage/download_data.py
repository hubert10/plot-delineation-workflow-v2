...
def create_directories(data_dir):
    ...
    DATASET_FOLDER = os.path.join(data_dir, "Data", "dataset")
    subfolder_names = ["f_predicted", "predicted", "test", "training", "validation"]
    for subfolder_name in subfolder_names:
        os.makedirs(os.path.join(DATASET_FOLDER, subfolder_name), exist_ok=True)
        if (
            (subfolder_name == "test")
            or (subfolder_name == "training")
            or (subfolder_name == "validation")
        ):
            os.makedirs(
                os.path.join(DATASET_FOLDER, subfolder_name, "images"), exist_ok=True
            )
            os.makedirs(
                os.path.join(DATASET_FOLDER, subfolder_name, "masks"), exist_ok=True
            )
    print("I have created all the directories needed for this workflow")
...
def download_satellite_image(out_path, save_path, api_key, filename):
    ...
    for i in range(len(db["x"])):
        base_url = "http://maps.googleapis.com/maps/api/staticmap?v=3.44&key={}&center={},{}&scale=2&format=jpg&zoom=18&size=1280x1280&sensor=false&maptype=satellite"
