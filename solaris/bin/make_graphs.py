...
def main():
    ...
    parser.add_argument(
        "--source_file", "-s", type=str, help="Full path to file to create graph from."
    )
    parser.add_argument(
        "--output_path",
        "-o",
        type=str,
        help="Full path to the output file for the graph" + " object.",
    )
    parser.add_argument(
        "--road_type_field",
        "-r",
    ...
    arg_df = arg_df.rename(
        columns={"source_file": "geojson", "first_edge_idx": "edge_idx"}
    )
    ...
    if not args.batch:
        ...
        with Pool(processes=args.workers) as pool:
            result = tqdm(
                pool.starmap(
                    _func_wrapper, zip(repeat(geojson_to_graph), arg_dict_list)
                )
            )
            pool.close()
