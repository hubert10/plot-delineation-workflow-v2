...
def main():
    ...
    parser.add_argument(
        "--source_file", "-s", type=str, help="Full path to file to create mask from."
    )
    parser.add_argument(
        "--reference_image",
        "-r",
    ...
    parser.add_argument(
        ...
        "-o",
        type=str,
        help="Full path to the output file for the converted" "footprints.",
    )
    parser.add_argument(
        "--geometry_column",
        "-g",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use this flag if the geometries"
    ...
    parser.add_argument(
        ...
        type=int,
        default=255,
        help="The value to set for labeled pixels in the" " mask. Defaults to 255.",
    )
    parser.add_argument(
        "--footprint",
        "-f",
        action="store_true",
        default=False,
        help="If this flag is set, the mask"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use this flag if any widths "
    ...
    if not args.batch:
        ...
        with Pool(processes=args.workers) as pool:
            result = tqdm(
                pool.starmap(_func_wrapper, zip(repeat(df_to_px_mask), arg_dict_list))
            )
            pool.close()
