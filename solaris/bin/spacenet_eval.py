...
def main():
    ...
    parser.add_argument("--proposal_csv", "-p", type=str, help="Proposal CSV")
    parser.add_argument("--truth_csv", "-t", type=str, help="Truth CSV")
    parser.add_argument(
        "--challenge",
        "-c",
        type=str,
    ...
    if args.challenge.lower() == "off-nadir":
        evalSettings = {"miniou": 0.5, "min_area": 20}
    ...
    if args.output_file:
        print(
            "Writing summary results to {}".format(
                args.output_file.rstrip(".csv") + ".csv"
            )
        )
        results_DF.to_csv(args.output_file.rstrip(".csv") + ".csv", index=False)
        print(
            "Writing full results to {}".format(
                args.output_file.rstrip(".csv") + "_full.csv"
            )
        )
        results_DF_Full.to_csv(
            args.output_file.rstrip(".csv") + "_full.csv", index=False
        )
