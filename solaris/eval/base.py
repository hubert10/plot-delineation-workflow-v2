...
class Evaluator:
    ...
    def eval_iou_spacenet_csv(
        ...
        for imageID in tqdm(imageIDList):
            ...
            proposal_GDF_copy = self.proposal_GDF[
                self.proposal_GDF[imageIDField] == imageID
            ].copy(deep=True)
            ...
            for _, pred_row in proposal_GDF_copy.iterrows():
                ...
                if pred_row.geometry.area > 0:
                    ...
                    iou_GDF = iou.calculate_iou(pred_poly, self.ground_truth_GDF_Edit)
                    # Get max iou
                ...
                if not proposal_GDF_copy.empty:
                    if iou_field in proposal_GDF_copy.columns:
                        TruePos = proposal_GDF_copy[
                            proposal_GDF_copy[iou_field] >= miniou
                        ].shape[0]
            ...
            FalseNeg = self.ground_truth_GDF_Edit[
                self.ground_truth_GDF_Edit.area > 0
            ].shape[0]
    ...
    def eval_iou(
        ...
        if calculate_class_scores:
            if not ground_truth_class_field:
                raise ValueError(
                    "Must provide ground_truth_class_field "
        ...
        for class_id in class_list:
            ...
            if class_id is not "all":  # this is probably unnecessary now
                self.ground_truth_GDF_Edit = self.ground_truth_GDF[
                    self.ground_truth_GDF[ground_truth_class_field] == class_id
                ].copy(deep=True)
            else:
                self.ground_truth_GDF_Edit = self.ground_truth_GDF.copy(deep=True)
    ...
    def eval_iou_return_GDFs(
        ...
        if calculate_class_scores:
            if not ground_truth_class_field:
                raise ValueError(
                    "Must provide ground_truth_class_field if using calculate_class_scores."
                )
            if class_list == ["all"]:
                class_list = list(
                    self.ground_truth_GDF[ground_truth_class_field].unique()
        ...
        if os.path.isfile(proposal_vector_file):
            ...
            if conf_field_mapping is not None:
                self.proposal_GDF["__max_conf_class"] = [
                    conf_field_mapping[item]
                    for item in self.proposal_GDF["__max_conf_class"].values
                ]
    ...
    def load_truth(
        self, ground_truth_vector_file, truthCSV=False, truth_geo_value="PolygonWKT_Pix"
    ):
...
def eval_base(
    ground_truth_vector_file, csvFile=False, truth_geo_value="PolygonWKT_Pix"
):
