...
def spacenet_buildings_2(
    prop_csv, truth_csv, miniou=0.5, min_area=20, challenge="spacenet_2"
):
    ...
    evaluator.load_proposal(prop_csv, conf_field_list=["Confidence"], proposalCSV=True)
    results = evaluator.eval_iou_spacenet_csv(
        miniou=miniou,
    ...
    results_DF_Full["AOI"] = [
        get_chip_id(imageID, challenge=challenge)
    ...
    for indexVal in results_DF.index:
        ...
        if float(rowValue["TruePos"] + rowValue["FalsePos"]) > 0:
            Precision = float(
                rowValue["TruePos"] / float(rowValue["TruePos"] + rowValue["FalsePos"])
            )
        else:
            Precision = 0
        ...
        if float(rowValue["TruePos"] + rowValue["FalseNeg"]) > 0:
            Recall = float(
                rowValue["TruePos"] / float(rowValue["TruePos"] + rowValue["FalseNeg"])
            )
        else:
            Recall = 0
...
def off_nadir_buildings(
    prop_csv, truth_csv, image_columns={}, miniou=0.5, min_area=20, verbose=False
):
...
def multi_temporal_buildings(
    prop_csv, truth_csv, miniou=0.25, min_area=4.0, beta=2.0, stats=False, verbose=False
):
    """
    ...
    # Load dataframes
    grnd_df = gpd.read_file(
        truth_csv, GEOM_POSSIBLE_NAMES="geometry", KEEP_GEOM_COLUMNS="NO"
    )
    prop_df = gpd.read_file(
        prop_csv, GEOM_POSSIBLE_NAMES="geometry", KEEP_GEOM_COLUMNS="NO"
    )
    if verbose:
    ...
    score, all_stats = scot_multi_aoi(
        grnd_df,
        prop_df,
        threshold=miniou,
        base_reward=100.0,
    ...
    if verbose:
        print('The submission "%s" receives a score of %f' % (prop_csv, score))
    if stats:
...
def get_chip_id(chip_name, challenge="spacenet_2"):
    ...
    if challenge in ["spacenet_2", "spacenet_3"]:
        ...
        chip_id = re.findall("Atlanta_nadir[0-9]{1,2}_catid_[0-9A-Z]{16}", chip_name)[0]
