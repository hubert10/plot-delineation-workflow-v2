...
def match_footprints(grnd_df, prop_df, threshold=0.25, base_reward=100.0):
    """
    ...
    grnd_id_to_index = {id: index for index, id in enumerate(sorted(list(grnd_id_set)))}
    prop_id_to_index = {id: index for index, id in enumerate(sorted(list(prop_id_set)))}
    ...
    intersect["iou"] = intersect["inter_area"] / (
        intersect["grnd_area"] + intersect["prop_area"] - intersect["inter_area"]
    )
    intersect["reward"] = intersect.apply(
        lambda row: (row.iou > threshold) * (base_reward + row.iou), axis=1
    )

    # Convert IOUs and rewards to 2D arrays (by way of sparse matrices)
    iou_matrix = scipy.sparse.coo_matrix(
        (intersect.iou, (intersect.index_1, intersect.index_2)),
        shape=(len(grnd_df), len(prop_df)),
    )
    iou_arr = iou_matrix.toarray()
    reward_matrix = scipy.sparse.coo_matrix(
        (intersect.reward, (intersect.index_1, intersect.index_2)),
        shape=(len(grnd_df), len(prop_df)),
    )
    reward_arr = reward_matrix.toarray()

    # Solve unbalanced linear assignment problem
    grnd_match, prop_match = scipy.optimize.linear_sum_assignment(
        reward_arr, maximize=True
    )
...
def scot_one_aoi(
    ...
    for i, timestep in enumerate(timesteps):
        ...
        (
            ...
            grnd_df_one_timestep,
            prop_df_one_timestep,
            threshold=threshold,
    ...
    for i in range(len(all_grnd_ids)):
        ...
        grnd_mismatch = (
            grnd_id in previous_grnd_ids
            and previous_prop_ids[previous_grnd_ids.index(grnd_id)] != prop_id
        )
        prop_mismatch = (
            prop_id in previous_prop_ids
            and previous_grnd_ids[previous_prop_ids.index(prop_id)] != grnd_id
        )
    ...
    if track_tp_net + (track_fp_net + track_fn_net) / 2.0 > 0:
        track_score = (track_tp_net) / (
            track_tp_net + (track_fp_net + track_fn_net) / 2.0
        )
    else:
        track_score = 0
    if change_tp_net + (change_fp_net + change_fn_net) / 2.0 > 0:
        change_score = (change_tp_net) / (
            change_tp_net + (change_fp_net + change_fn_net) / 2.0
        )
    else:
        change_score = 0
    if beta * beta * change_score + track_score > 0:
        combo_score = (
            (1 + beta * beta)
            * (change_score * track_score)
            / (beta * beta * change_score + track_score)
        )
    else:
        combo_score = 0
    if verbose:
