...
class KerasTerminateOnMetricNaN(Callback):
        ...
        if self.ckpt == "epoch":
            ...
            if self.metric is not None:
                if np.isnan(metric_score) or np.isinf(metric_score):
                    print(
                        "Epoch {}: Invalid score for metric {}, terminating"
    ...
    def on_batch_end(self, batch, logs=None):
        if self.ckpt == "batch":
            ...
            if np.isnan(metric_score) or np.isinf(metric_score):
                print(
                    "Batch {}: Invalid score for metric"
...
def get_lr_schedule(framework, config):
    ...
    schedule_type = config["training"]["callbacks"]["lr_schedule"]["schedule_type"]
    initial_lr = config["training"]["lr"]
    update_frequency = config["training"]["callbacks"]["lr_schedule"].get(
        "update_frequency", 1
    )
    factor = config["training"]["callbacks"]["lr_schedule"].get("factor", 0)
    schedule_dict = config["training"]["callbacks"]["lr_schedule"].get(
        "schedule_dict", None
    )
    if framework == "keras":
        lr_sched_func = keras_lr_schedule(
            schedule_type, initial_lr, update_frequency, factor, schedule_dict
        )
        ...
        if config["training"]["callbacks"]["lr_schedule"]["schedule_type"] == "linear":
            lr_scheduler = torch.optim.lr_scheduler.StepLR
        elif (
            config["training"]["callbacks"]["lr_schedule"]["schedule_type"]
            == "exponential"
        ):
...
def keras_lr_schedule(
    schedule_type, initial_lr=0.001, update_frequency=1, factor=0, schedule_dict=None
):
    ...
    if schedule_type == "arbitrary":
        if schedule_dict is None:
            raise ValueError(
                "If using an arbitrary schedule, an epoch: lr " "dict must be provided."
            )
        lookup_dict = {}
