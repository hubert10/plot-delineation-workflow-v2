...
def make_data_generator(framework, config, df, stage="train"):
    ...
    if framework.lower() not in ["keras", "pytorch", "torch"]:
        raise ValueError(
            "{} is not an accepted value for `framework`".format(framework)
        )
...
class KerasSegmentationSequence(keras.utils.Sequence):
    ...
    def _data_generation(self, image_idxs):
        ...
        X = np.empty((self.batch_size, self.height, self.width, self.input_channels))
        if self.label_type == "mask":
            y = np.empty(
                (self.batch_size, self.height, self.width, self.output_channels)
            )
        else:
...
class TorchDataset(Dataset):
    ...
    def __init__(
        ...
        # if it's a string, get the appropriate object
            ...
            try:
                ...
                raise ValueError("The data type {} is not supported".format(dtype))
    ...
    def __getitem__(self, idx):
        ...
        sample["image"] = _check_channel_order(sample["image"], "torch").astype(
            self.dtype
        )
        sample["mask"] = _check_channel_order(sample["mask"], "torch").astype(
            np.float32
        )
        return sample
...
class InferenceTiler(object):
    ...
    def __init__(
        self, framework, width, height, x_step=None, y_step=None, augmentations=None
    ):
    ...
    def __call__(self, im):
        ...
        for y in range(y_steps):
            ...
            for x in range(x_steps):
                ...
                subarr = im[y_min : y_min + self.height, x_min : x_min + self.width, :]
