...
class Inferer(object):
    ...
    def __init__(self, config, custom_model_dict=None):
        ...
        if self.config["train"]:
            warn(
                "Because the configuration specifies both training and "
            ...
            if custom_model_dict is not None:
                custom_model_dict["weight_path"] = self.config["training"][
                    "model_dest_path"
                ]
        else:
        ...
        self.window_step_x = self.config["inference"].get("window_step_size_x", None)
        self.window_step_y = self.config["inference"].get("window_step_size_y", None)
    ...
    def __call__(self, infer_df=None):
        ...
        inf_tiler = InferenceTiler(
            ...
            augmentations=process_aug_dict(self.config["inference_augmentation"]),
        )
        for idx, im_path in enumerate(infer_df["image"]):
            ...
            if self.framework == "keras":
                subarr_preds = self.model.predict(inf_input, batch_size=self.batch_size)
                ...
                if self.config["data_specs"].get("additional_inputs", None) is not None:
                    inf_input = [inf_input]
                    for i in self.config["data_specs"]["additional_inputs"]:
                        inf_input.append(infer_df[i].iloc[idx].to(device))

                subarr_preds = self.model(inf_input)
                subarr_preds = subarr_preds.cpu().data.numpy()
            stitched_result = stitch_images(
                subarr_preds,
                idx_refs=idx_refs,
            ...
            create_multiband_geotiff(
                stitched_result,
                os.path.join(self.output_dir, os.path.split(im_path)[1]),
                proj=proj,
                geo=gt,
                nodata=np.nan,
