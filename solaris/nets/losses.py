...
def get_loss(framework, loss, loss_weights=None, custom_losses=None):
    ...
    if not isinstance(loss, dict):
        raise TypeError(
            "The loss description is formatted improperly." " See the docs for details."
        )
    if len(loss) > 1:
        ...
        if list(loss.keys()).sort() != list(weights.keys()).sort():
            raise ValueError("The losses and weights must have the same name keys.")
    ...
    def composite(y_true, y_pred):
        loss = K.sum(
            K.flatten(
                K.stack(
                    [
                        weight_dict[loss_name]
                        * get_single_loss(
                            "keras", loss_name, loss_params, custom_losses
                        )(y_true, y_pred)
...
class TorchCompositeLoss(nn.Module):
    ...
    def __init__(self, loss_dict, weight_dict=None, custom_losses=None):
        ...
        self.losses = {
            loss_name: get_single_loss("pytorch", loss_name, loss_params, custom_losses)
            for loss_name, loss_params in loss_dict.items()
        }
