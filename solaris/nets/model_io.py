...
def get_model(
    ...
    if custom_model_dict is not None:
        ...
        if md is None:  # if the model's not provided by solaris
            raise ValueError(
                f"{model_name} can't be found in solaris and no "
    ...
    if model is not None and pretrained:
        try:
            ...
            warn(
                f"The model weights file {model_path} was not found."
...
def _reset_torch_weights(torch_layer):
    if isinstance(torch_layer, torch.nn.Conv2d) or isinstance(
...
def _download_weights(model_dict):
    ...
    weight_dest_path = model_dict.get(
        "weight_path", os.path.join(weights_dir, weight_url.split("/")[-1])
    )
    if weight_url is None:
        ...
        if r.status_code != 200:
            raise ValueError(
                "The file could not be downloaded. Check the URL"
