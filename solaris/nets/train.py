...
class Trainer(object):
    ...
    def __init__(self, config, custom_model_dict=None, custom_losses=None):
        ...
        self.train_datagen = make_data_generator(
            self.framework, self.config, self.train_df, stage="train"
        )
        self.val_datagen = make_data_generator(
            self.framework, self.config, self.val_df, stage="validate"
        )
        ...
        self.checkpoint_frequency = self.config["training"].get(
            "checkpoint_" + "frequency"
        )
    ...
    def initialize_model(self):
        ...
        if self.framework == "keras":
            self.model = self.model.compile(
                optimizer=self.optimizer, loss=self.loss, metrics=self.metrics["train"]
            )
            ...
            for cb in self.callbacks:
                if isinstance(cb, _LRScheduler):
                    self.optimizer = cb(
                        self.optimizer,
                        **self.config["training"]["callbacks"]["lr_schedule"].get(
                            ["schedule_dict"], {}
                        )
                    )
            ...
            for epoch in range(self.epochs):
                ...
                for batch_idx, batch in enumerate(self.train_datagen):
                    if torch.cuda.is_available():
                        if (
                            self.config["data_specs"].get("additional_inputs", None)
                            is not None
                        ):
                            data = []
                            for i in ["image"] + self.config["data_specs"][
                                "additional_inputs"
                            ]:
                    ...
                    if self.verbose and batch_idx % 10 == 0:
                        ...
                        print(
                            "    loss at batch {}: {}".format(batch_idx, loss),
                            flush=True,
                        )
                ...
                check_continue = self._run_torch_callbacks(
                    loss.detach().cpu().numpy(), val_loss.detach().cpu().numpy()
                )
    ...
    def save_model(self):
        ...
        if self.framework == "keras":
            ...
            if isinstance(self.model, torch.nn.DataParallel):
                ...
                torch.save(
                    self.model.state_dict(), self.config["training"]["model_dest_path"]
                )
...
def get_train_val_dfs(config):
    ...
    if config["data_specs"]["val_holdout_frac"] is None:
        ...
        val_subset = np.random.choice(
            train_df.index, int(len(train_df) * val_frac), replace=False
        )
