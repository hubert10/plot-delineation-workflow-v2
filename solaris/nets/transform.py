...
class SwapChannels(ImageOnlyTransform):
    ...
    def __init__(self, first_idx, second_idx, axis=0, always_apply=False, p=1.0):
        super().__init__(always_apply, p)
        if axis not in [0, 2]:
            raise ValueError(
                "Solaris can only accommodate axis values of 0 "
...
class Rotate(DualTransform):
    ...
    def __init__(
        self, limit=90, border_mode="reflect", cval=0.0, always_apply=False, p=0.5
    ):
    ...
    def apply(self, im_arr, angle=0, border_mode="reflect", cval=0, **params):
        return ndi.interpolation.rotate(
            im_arr, angle=angle, mode=self.border_mode, cval=self.cval
        )
...
class RandomScale(DualTransform):
    ...
    def __init__(
        ...
        if self.scale_limit[0] == -self.scale_limit[1]:
            self.scale_limit = tuple([self.scale_limit[0] + 1, self.scale_limit[1] + 1])
...
def scale(im, scale_x, scale_y, interpolation):
    ...
    return np.array(
        Image.fromarray(im.astype("uint8")).resize((x_size, y_size), interpolation)
    )
