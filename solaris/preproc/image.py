    ...
    def transform(self, pin):
        ...
        if self.save_projection:
            ...
            if proj_lens[0] > 0 and proj_lens[0] >= proj_lens[1]:
                ...
                dataset.SetGCPs(pin.metadata["gcps"], pin.metadata["gcp_projection"])
        ...
        props = pd.DataFrame(
            {
                ...
                "pos": np.count_nonzero(np.nan_to_num(pin.data, nan=-1.0) > 0, (1, 2)),
                "zero": np.count_nonzero(pin.data == 0, (1, 2)),
                "neg": np.count_nonzero(np.nan_to_num(pin.data, nan=1.0) < 0, (1, 2)),
...
class Crop(PipeSegment):
    ...
    def transform(self, pin):
        ...
        if self.mode in ["pixel", "p", 0]:
            srcWin = [col_min, row_min, col_max - col_min + 1, row_max - row_min + 1]
            projWin = None
