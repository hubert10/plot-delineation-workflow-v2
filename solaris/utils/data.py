...
def make_dataset_csv(
    ...
    im_fnames = get_files_recursively(
        im_dir, traverse_subdirs=recursive, extension=im_ext
    )
    ...
    if stage != "infer":
        ...
        label_fnames = get_files_recursively(
            label_dir, traverse_subdirs=recursive, extension=label_ext
        )
        ...
        if match_re:
            ...
            label_match_strs = temp_label_df["label_fname"].str.extract(match_re)
            if len(im_match_strs.columns) > 1 or len(label_match_strs.columns) > 1:
                raise ValueError(
                    "Multiple regex matches occurred within " "individual filenames."
                )
            else:
        ...
        logger.debug("Aligning label and image dataframes by" " match_str.")
        temp_join_df = pd.merge(temp_im_df, temp_label_df, on="match_str", how="inner")
        logger.debug(f"Length of joined dataframe: {len(temp_join_df)}")
        if len(temp_join_df) < len(temp_im_df) and ignore_mismatch is None:
            raise ValueError(
                "There is not a perfect 1:1 match of images "
            ...
            raise ValueError(
                "There are multiple label files matching at " "least one image file."
            )
        elif len(temp_join_df) > len(temp_im_df) and ignore_mismatch == "skip":
            logger.info(
                'ignore_mismatch="skip", so dropping any images with '
            ...
            logger.info(
                "Remaining images after dropping duplicates: " f"{len(temp_join_df)}"
            )
