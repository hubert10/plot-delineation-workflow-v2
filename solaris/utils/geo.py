...
def _reproject(
    ...
    if input_type == "vector":
        ...
        if isinstance(input_data, rasterio.DatasetReader):
            ...
            if dest_path is not None:
                gdal.Warp(
                    dest_path, input_data, dstSRS="EPSG:" + str(target_crs.to_epsg())
                )
                output = gdal.Open(dest_path)
            else:
                raise ValueError(
                    "An output path must be provided for " "reprojecting GDAL datasets."
                )
    return output
...
def get_crs(obj):
    ...
    if isinstance(obj, gpd.GeoDataFrame):
        ...
        return _check_crs(
            int(
                osr.SpatialReference(wkt=obj.GetProjection()).GetAttrValue(
                    "AUTHORITY", 1
                )
            )
        )
...
def _parse_geo_data(input):
    if isinstance(input, str):
        ...
        if isinstance(input_data, pd.DataFrame):
            input_type = "vector"
        elif isinstance(input_data, rasterio.DatasetReader) or isinstance(
