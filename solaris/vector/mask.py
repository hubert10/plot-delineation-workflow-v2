    ...
    **kwargs : optional
    ...
    if footprint_msk is None:
        footprint_msk = footprint_mask(
            reference_im=reference_im, burn_value=burn_value, **kwargs
        )
...
def contact_mask(
    ...
    if intersect_poly.is_empty:
        ...
        df_for_footprint = pd.DataFrame(
            {"shape_name": ["overlap"], "geometry": [intersect_poly]}
        )
        # catch bowties
...
def road_mask(
    ...
    if burn_field:
        if out_type == "int":
            feature_list = list(
                zip(buffered_df[geom_col], buffered_df[burn_field].astype("uint8"))
            )
        else:
            feature_list = list(
                zip(buffered_df[geom_col], buffered_df[burn_field].astype("uint8"))
        ...
        feature_list = list(zip(buffered_df[geom_col], [burn_value] * len(buffered_df)))

    output_arr = features.rasterize(
        shapes=feature_list, out_shape=shape, transform=affine_obj
    )
    if min_background_value:
        output_arr = np.clip(output_arr, min_background_value, np.max(output_arr))
...
def buffer_df_geoms(
    df, buffer, meters=False, reference_im=None, geom_col="geometry", affine_obj=None
):
    ...
    if not meters:
        if hasattr(df, "crs") and reference_im is not None:
            ...
            df_for_buffer = affine_transform_gdf(df.copy(), affine_obj=affine_obj)
        ...
        if hasattr(df, "crs"):
            ...
            if reference_im is not None:
                df_for_buffer = georegister_px_df(df.copy(), im_path=reference_im)
            else:
                raise ValueError(
                    "If using `meters=True`, either `df` must be "
                    "a geopandas GeoDataFrame or `reference_im` "
                    "must be provided for georegistration."
                )

    df_for_buffer[geom_col] = df_for_buffer[geom_col].apply(lambda x: x.buffer(buffer))

    # return to original crs
    if _check_crs(getattr(df_for_buffer, "crs", None)) != orig_crs:
        if orig_crs is not None and getattr(df_for_buffer, "crs", None) is not None:
            ...
            buffered_df = georegister_px_df(
                df_for_buffer, im_path=reference_im, affine_obj=affine_obj, crs=orig_crs
            )
    else:
...
def preds_to_binary(pred_arr, channel_scaling=None, bg_threshold=0):
    ...
    if len(pred_arr.shape) == 3:
        ...
        if channel_scaling is None:  # if scale values weren't provided
            channel_scaling = np.ones(shape=(pred_arr.shape[-1]), dtype="float")
...
def mask_to_poly_geojson(
    ...
    **kwargs
    ...
    if do_transform and reference_im is None:
        raise ValueError("Coordinate transformation requires a reference image.")
...
def crs_is_metric(gdf):
    ...
    if units in ['"meter"', '"metre"', "'meter'", "'meter'", "meter", "metre"]:
        return True
    else:
        return False
...
def instance_mask(
    ...
    if out_type == "int":
        output_arr = np.empty(
            shape=(shape[0], shape[1], len(feature_list)), dtype="uint8"
        )
    else:
        output_arr = np.empty(
            shape=(shape[0], shape[1], len(feature_list)), dtype="float32"
        )
    # initialize the output array

    for idx, feat in enumerate(feature_list):
        output_arr[:, :, idx] = features.rasterize(
            [feat], out_shape=shape, transform=affine_obj
        )

    if reference_im:
        reference_im = _check_rasterio_im_load(reference_im)
    try:
        bad_data_mask = (reference_im.read() == reference_im.nodata).any(
            axis=0
        )  # take logical and along all dims so that all pixxels not -9999 across bands
    except AttributeError as ae:  # raise another, more verbose AttributeError
        raise AttributeError(
            "A nodata value is not defined for the source image. Make sure the reference_im has a nodata value defined."
        ) from ae
    if len(bad_data_mask.shape) > 2:
        bad_data_mask = np.dstack([bad_data_mask] * output_arr.shape[2])
        output_arr = np.where(
            bad_data_mask, 0, output_arr
        )  # mask is broadcasted to filter labels where there are non-nan image values
...
def geojsons_to_masks_and_fill_nodata(rtiler, vtiler, label_tile_dir, fill_value=0):
    ...
    for img_tile, geojson_tile in tqdm(
        zip(sorted(rtiler.tile_paths), sorted(vtiler.tile_paths))
    ):
        ...
        arr = instance_mask(
            gdf,
            out_file=rasterized_label_path,
            reference_im=img_tile,
            geom_col="geometry",
            do_transform=None,
            out_type="int",
            burn_value=1,
            burn_field=None,
        )  # this saves the file, unless it is empty in which case we deal with it below.
        if (
            not arr.any()
        ):  # in case no instances in a tile we save it with "empty" at the front of the basename
            ...
            if isinstance(meta["nodata"], float):
                meta.update(nodata=0)
            rasterized_label_path = os.path.join(
                label_tile_dir, "empty_" + fid + ".tif"
            )
